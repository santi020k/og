...
synchronized examples and complete v0.7 references for metadata, JSON-LD, built-site auditing,
...
and remaining custom-renderer responsibilities. `santi-og upgrade --to 0.7.0` updates regular
...
`check`, `compare`, `migrate`, and `upgrade` support machine-readable JSON summaries.
